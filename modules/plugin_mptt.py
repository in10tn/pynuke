...
class MPTT(object):
    ...
    def define_tables(self, migrate=True, fake_migrate=False):
        ...
        if not settings.table_node_name in db.tables:
            db.define_table(
                ...
                Field('parent', 
                       'reference %s' % settings.table_node_name,
                        label=T("Parent page"),
                        comment=T("Select the page that you would like this page to be a child of."),
                        ),
    ...
    def descendants_from_node(self, node_or_node_id, include_self=False,
        ...
        query = (table_node.lft > left) & (table_node.rgt < right)
        query = query & (table_node.tree_id == node.tree_id)
        if not includedeleted:
            query = query & ((table_node.isdeleted==None) | (table_node.isdeleted==False)) 

        if onlyvisibles:
            query = query & (table_node.isvisible == True)

        if onlyactivated:
            query = query = query & ((table_node.disablelink==None) | (table_node.disablelink==False))
    ...
    def delete_node(self, node_or_node_id):
        ...
        db(table_node.lft >= left)(table_node.lft <= right)(
           ...
           table_node.tree_id == current_tree_id).update(isdeleted=True)
           
        db(table_node.rgt >= node.rgt)(table_node.tree_id == current_tree_id
           )((table_node.isdeleted == False) | (table_node.isdeleted == None)).update(rgt=table_node.rgt - tree_width)
        db(table_node.lft >= node.rgt)(table_node.tree_id == current_tree_id
           )((table_node.isdeleted == False) | (table_node.isdeleted == None)).update(lft=table_node.lft - tree_width)
